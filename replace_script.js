...
const shapesJson = `const animalShapes = {
...
const startMarker = 'const animalShapes = {';
const endMarker = 'function draw() {'; // Just to find the block end safely, drawStroke is right before this

let startIndex = code.indexOf(startMarker);
let endIndex = code.indexOf(endMarker);

if (startIndex > -1 && endIndex > -1) {
    // we want to replace up to right before `function draw() {`
    // but preserving any newlines
    let replacedCode = code.substring(0, startIndex) + shapesJson + '\n\n' + code.substring(endIndex);
    fs.writeFileSync('index.html', replacedCode);
    console.log("Success!");
} else {
